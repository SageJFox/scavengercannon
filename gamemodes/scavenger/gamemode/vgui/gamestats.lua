...
local PANEL = {}
	function PANEL:Init()
		self:SetTitle("#scav.round.winner.player")
	...
	function PANEL:InvalidateLayout()
		if not self.initalized then return end
	end
...
local PANEL = {}
	function PANEL:Init()
		self:SetTitle("#scav.round.winner.team")
		self.NameLabel = vgui.Create("DLabel", self)
			self.NameLabel:SetFont("DermaLarge")
		self.Initialized = true
	...
	function PANEL:SetTeam(teamid)
		...
		local mul = 0.7 --GetConVarNumber("scav_col_team_mul")
		col.r = col.r*mul
		col.g = col.g*mul
		col.b = col.b*mul
		self.m_bgColor = col
...
function PANEL:SetPlayer(pl)
	...
	self.NameLabel:SetText(pl:Nick())
	self.ScoreLabel:SetText(ScavLocalize("scav.score", pl:Frags()))
end
