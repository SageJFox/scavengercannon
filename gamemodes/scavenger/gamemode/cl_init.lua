...
function GM:ShowMenuInstructions()
	...
	if panel then
		...
		panel:SetText(ScavLocalize("scav.menu.instruction", menukey and string.upper(menukey) or ScavLocalize("scav.nokey")))
...
local function getweapontoswitchto(pl, slot)
	...
	for _, v in pairs(pl:GetWeapons()) do
		if v.Slot ~= slot then continue end
...
function GM:PlayerBindPress(pl, bind, pressed)
	if not slottranslate[bind] then return false end
