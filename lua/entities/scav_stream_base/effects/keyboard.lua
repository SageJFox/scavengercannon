...
function ENT:ScavStreamKeyboardSound()
	local snd = math.random(#self.sound)
	if self.sound[snd] then
		self.sound[snd]:Stop()
		self.sound[snd]:Play()
	end
	timer.Create(tostring(self) .. "ScavStreamKeyboardSound", 0.375, 1, function()
		if not IsValid(self) then return end
		self:ScavStreamKeyboardSound()
	end)
end
...
scripted_ents.Register(ENT, "scav_stream_keyboard")
