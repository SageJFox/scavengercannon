...
function ENT:Think()
	...
	local vel_scale = math.Rand(0.5, 0.666667) / math.Clamp(self:GetVelocity():Length(), 1, 10)
...
if SERVER then
	function ENT:SetForce(force, mul)
		...
		local phys = self:GetPhysicsObject()
		if not IsValid(phys) then return Msg("Warning: [scav_projectile_canister] Physics object isn't valid!\n") end
		...
		self:SetOffset(mul > 0 and self.ThrustOffset or self.ThrustOffsetR)
	...
	function ENT:AddMul(mul, bDown)
		...
		self.Multiply = self.Multiply ~= mul and mul or 0
...
function ENT:PhysicsCollide(data, phys)
	...
	if other:IsWorld() then
		if data.Speed > 300 then
			local damage = math.floor(data.Speed * 0.001)
			if damage > 0 then
				self:TakeDamage(damage, self.Owner, other)
			end
		end
	else
		if data.Speed > 100 then
			local damage = math.floor(data.Speed * 0.005)
			if damage > 0 then
				self:TakeDamage(damage, self.Owner, other)
			end
		end
