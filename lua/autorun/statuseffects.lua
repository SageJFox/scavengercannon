...
local STATUS = {}
	...
	local UpdateEnemies = function(self, ignore)
		if not SERVER then return end
		if ignore then
			hook.Add("OnEntityCreated", "ScavUpdateCloak_" .. tostring(self.Owner), function(ent)
				timer.Simple(0.125, function()
					if not self or not IsValid(ent) or not ent:IsNPC() then return end

					local relationship = ent:Disposition(self.Owner)
					if relationship == D_ER or relationship == D_LI then return end
					
					ent:MarkEnemyAsEluded(self.Owner)
					ent:IgnoreEnemyUntil(self.Owner, self.EndTime)
				end)
			end)
		else
			hook.Remove("OnEntityCreated", "ScavUpdateCloak_" .. tostring(self.Owner))
		end

		for _, npc in ents.Iterator() do
			if not IsValid(npc) or not npc:IsNPC() then continue end

			local relationship = npc:Disposition(self.Owner)
			if relationship == D_ER or relationship == D_LI then continue end

			if ignore then
				npc:MarkEnemyAsEluded(self.Owner)
				npc:IgnoreEnemyUntil(self.Owner, self.EndTime)
			else
				npc:IgnoreEnemyUntil(self.Owner, 0)
			end
		end
	end
	...
	function STATUS:Finish()
		...
		self.Owner:EmitSound(TF2 and "player/spy_uncloak.wav" or "npc/turret_floor/die.wav")
	...
	function STATUS:Add(duration, value)
		self.EndTime = self.EndTime + duration
		UpdateEnemies(self, self.EndTime > CurTime())
	end
...
local STATUS = {}
	...
	if CLIENT then
		function STATUS:Think()	
			if (self.Owner == LocalPlayer()) then
				...
				self.Owner:SetEyeAngles((VectorRand(-0.02, 0.02) + self.Owner:GetAimVector()):Angle())
			end
	...
	hook.Add("GetMotionBlurValues", "ShockBlur", function(x, y, fwd, spin)
		if GetViewEntity().Status_shock then
			return math.Rand(-1, 1), math.Rand(-1, 1), fwd, spin
		end
	end)
...
local STATUS = {}
	...
	if SERVER then
		hook.Add("EntityTakeDamage", "ScavStatusInvuln", function(ent, dmginfo)
...
local STATUS = {}
	...
	function STATUS:Think()
		if SERVER then
			...
			if self.nextclick < CurTime() and
				LocalPlayer():GetPos():Distance(self.Owner:GetPos()) < 1000 and
				math.random(0, math.ceil(LocalPlayer():GetPos():Distance(self.Owner:GetPos()) / 100)) < 2 then
	...
	--Clean up world radiation spots
	function STATUS:Finish()
		if not SERVER then return end
		if not IsValid(self.Owner) then return end
		if self.Owner:GetName() == "ScavGun_GammaRay_WorldSpot" then
			self.Owner:Fire("kill")
		end
	end
